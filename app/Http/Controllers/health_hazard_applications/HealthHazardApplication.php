...
class HealthHazardApplication extends Controller
{
    ...
    public function HealthHazardApplicationShowDetails()
    {
        ...
        if (!empty($forms)) {
            foreach ($forms as $rs) {
                $rs->setAttribute('appointmentte', HealthLicenseAppointmentLogs::orderBy('id', 'desc')->first());
                $rs->setAttribute('payment', HealthLicensePaymentLogs::orderBy('id', 'desc')->first());
            }
        }
    ...
    public function HealthHazardApplicationUserExportPDF($id)
    {
        ...
        $pdf = Pdf::loadView(
            'admin.health_hazard_applications.form.pdf-form',
            compact('form')
